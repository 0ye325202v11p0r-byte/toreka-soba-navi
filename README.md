...
- ✅ **Phase 1: コアアプリ**（DB・認証・マーケット一覧・個人ポートフォリオ）— 完成・動作確認済み
  - Supabase（Postgres + Auth）に408件のカードデータを収録（移行時379件 + 「世界最強の戦士」セットから新規29件追加、2026-09-11）
  - ログイン（メールのマジックリンク）
...
- ✅ **Phase 2: 価格自動更新**（日次クロン）— 実装・動作確認済み
  - `/api/cron/refresh-prices` が407/408件のカードを自動追跡（残り1件=c9は印刷バリエーションの実測データが存在しないため意図的に対象外）
