...
**⚠️ これは本番（https://toreka-soba-navi.vercel.app、README.mdの記載上は既にデプロイ・稼働中）に現在も存在すると推測される、実際に悪用可能な穴です。** ただし念のため明記します——**この「本番に存在する」という表現は、README.mdの記載とローカルの`supabase/schema.sql`/コードを読んだ上での推測であり、本番のSupabase/Vercel環境を直接クエリ・ログインして確認した結果ではありません**（Codex独立レビュー、2026-09-13の指摘を受けて追記。このセッション・Codexとも本番環境への直接アクセスは行っていません）。`/admin/sync-status`は「ログイン済みかどうか」しかチェックしておらず、「管理者かどうか」は一度も検証していませんでした。本サイトのログインはパスワードレスのメールリンク方式（`signInWithOtp`）で許可リストも無いため、**任意のメールアドレスで誰でも新規登録した瞬間に、このページ（クロンの実行履歴・成功/失敗件数・内部カードIDを含む生のエラーサンプル）が閲覧できてしまう状態**だったはずです（ローカルコードの推測）。
...
**⚠️ 2026-09-13追記（Codex独立レビューで発見）：ページ側判定（`isAdminUser()`）とDB側RLSポリシーで、メールアドレスの正規化仕様が食い違っていました。** `isAdminUser()`は前後空白trim＋大小文字無視で比較する一方、当初のRLSポリシーは完全一致（`=`）でした。同じ`ADMIN_EMAIL`でも、`.env.local`/Vercelに入力した際の大小文字や空白の違い次第で、ページは表示されるのにDBは0件を返す——**情報漏洩ではなく、管理者本人が締め出される自己ロックアウト**が起こり得る状態でした。両ポリシーを`lower(trim(...))`で正規化し修正済み（下記SQLに反映済み）。また、ポリシー名の重複エラーでメールアドレスのローテーション時に再実行できない問題も合わせて修正し、`drop policy if exists`を追加しています。

**有効化に必要な作業（このセッションでは実行不可・ユーザー自身の対応が必要）：**
1. Supabaseダッシュボードで、`migration/retrofit_admin_only_sync_runs.sql`の`'REPLACE_WITH_YOUR_ADMIN_EMAIL'`を実際の管理者メールアドレスに置き換えてから、SQL Editorで実行する（既存の`sync_runs`テーブル向け——`yuyutei_sync_runs`は上記手順1でschema.sqlから新規作成する際に同様の置き換えを行う）。正規化により大小文字・前後空白は自動で吸収されるが、貼り付け時の余分な空白等は避けるのが望ましい。
...
| `expand_catalog.mjs` | `new_candidates.json`の各URLを実際にfetchし、ページに埋め込まれたChart.jsの生データ（labels/data配列、通常25〜30週分の実測価格）を正規表現で直接パースして、WebFetch要約より遥かに正確な価格履歴を取得。本番cronと同じ礼儀正しいレート制限（1.2〜1.3秒間隔・正直なUser-Agent）でSupabaseに投入する。`--start N` `--limit N`で分割実行・再開が可能。⚠️ **2026-09-12セルフレビューで発見・修正：** このスクリプト自身が`computeStats`/`buildVerdictText`のローカルコピーを持っており、いずれも**修正前（avg30/90が暦日ではなく配列末尾30/90件を使う旧バグ・trend90の動詞をpctVsAvg90ではなくjudgmentから選ぶ旧バグ）のまま**放置されていた。本スクリプトのコメント自身が「通常25〜30週分」と明記する疎な履歴はまさにavg30/90バグを誘発する条件そのもので、再実行すれば新規カードに対して同じバグ（実測844件のうち52%の数値がずれ、27%は判定自体が誤っていた既知のインシデント）を再現しうる状態だった。実際に7ヶ月分・30週間隔の履歴で検証したところ、旧ロジックはavg30とavg90が完全に同一の値（全期間平均）になってしまうことを確認。`scrape_yuyutei.mjs`が`yuyuteiParser.ts`を共用インポートしているのと同じ方式で、`src/lib/priceStats.ts`/`src/lib/ai-verdict.ts`の実体をimportするよう修正し、ローカルコピーを削除した | カードデータをさらに拡充する際の主力スクリプト。2026-09-11時点で`sitemap-cards.xml`の全894件（23の特別収録ボックス分）を処理済み＝このデータソースからは事実上枯渇状態。**このサイトは「特別収録ボックス」のパラレル/チェイスカードのみを個別ページ化しており、OP01〜OP17などメインブースターの通常レアリティ（ノーマルのC/UC/R/SR等）は元々サイトに個別ページが存在しない**（`sitemap-expansions.xml`で全23セットを確認済み・他に隠れたセットなし）。この通常レアリティのギャップは`scrape_yuyutei.mjs`（第2のデータソース）で埋めた |
