...
**`isMissingTableError()`の精度（2026-09-12、Codexの2回目の独立再検証を受けて改訂）：** 初版は`message.includes("does not exist")`／`message.includes("schema cache")`という単純な部分文字列一致で「テーブル未作成」を判定していましたが、これは広すぎました。例えば`app_settings`テーブル自体は存在していても、**列**が見つからない場合のPostgRESTエラー（`PGRST204`、メッセージに"schema cache"を含む）まで「未設定」と誤判定し、スクレイプを許可してしまう欠陥がありました。修正後は、①`code`が`42P01`／`PGRST205`（テーブル欠落の既知コード。`PGRST202`は関数欠落のコードであり誤って含まれていたのを削除）**かつ**メッセージに`app_settings`という名前が含まれる場合、②または（codeが無い場合のフォールバックとして）メッセージが`app_settings`という名前**と**「table/relation」＋「does not exist/could not find」の両方を含む場合、の2条件を満たした時だけ「unconfigured」と判定します。列欠落・別テーブルのエラー・関数欠落エラー等は全て`unknown`（フェイルクローズ）に倒れるよう修正しました。

**検証：** `migration/verify_app_settings.mjs`（26アサーション、4状態の判定・isYuyuteiSourceEnabled/canScrapeYuyuteiの使い分け・**列欠落エラー/別テーブルのエラー/関数欠落エラーが正しく`unknown`に分類されること**を検証）。`migration/verify_refresh_yuyutei_prices.mjs`にシナリオ追加（false・一般的なクエリエラー・例外・タイムアウト・不正値・**列欠落エラー・別テーブルのエラー**の7パターン全てで実際のfetch回数が0であることを確認、加えてテーブル未作成状態では通常通り57セット全て実行されることも確認）。計68アサーション全PASS。
